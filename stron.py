...
def clone():
    ...
    try:
        ...
        f.close()
    print("Proxys baixadas com sucesso! !")
...
def main():
    ...
    ip = str(input("IP ou URL:"))
    if ip == "":
        print("Errado.. Tente noValente!")
        return main()
    page = str(input("PAGINA: (default=/):"))
    if page == "":
        page = "/"
    port = str(input("PORTA: (HTTPS=443):"))
    if port =="":
        port = 80
        print("Port 80 Selecionada")
    else:
        port = int(port)
    if port == 443:
        print("Port 443 Selecionada")        
    ...
    if th_num == "":
        ...
        th_num = int(th_num)
    #if mode == "get" or mode == "GET":
    N = str(input("Deseja baixar as proxys? (se não usar o ataque será fraco. ?(y/n):"))
    if N == "n" or N =="N":
        pass
    else:
        clone()
